...
/** What a host with a table for the log does, without the SQL. Cursor is 1-based like SqliteSessionLog's. */
...
async function bootHost(store?: SessionLogStore): Promise<Host> {
	...
	const preset = createTestPreset({ id: 'logs-test', workspaceDir: `${basePath}/workspace` })
...
describe('logs.tail on a host with a store', () => {
	test('never touches the filesystem', async () => {
