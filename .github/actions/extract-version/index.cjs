...
;(() => {
	...
	exec('git tag -l', (err, stdout) => {
		...
		const sameIsStable = gitVersions.filter((it) => Boolean(it.prerelease) === Boolean(version.prerelease))
		...
		const samePatchAndStability = sameMinor.filter(
			(it) => it.patch === version.patch && it.prerelease === version.prerelease,
		)
		const latestPrereleaseVersion =
			version.prerelease && samePatchAndStability.length > 0
		...
		if (version.prerelease) {
			latestVersions.push(
				`${version.major}.${version.minor}.${version.patch}-${version.prerelease}.${version.prereleaseVersion}`,
			)
		}
